...
public class SetWebFlag extends ActionExecuterAbstractBase {
	...
	protected void executeImpl(Action action, NodeRef actionedUponNodeRef) {
		...
		properties.put(QName.createQName(Model.NAMESPACE_CONTENT_MODEL, Model.PROP_IS_ACTIVE), activeFlag);
		  
		if (activeFlag) {
			properties.put(QName.createQName(Model.NAMESPACE_CONTENT_MODEL, Model.PROP_PUBLISHED), new Date());
		}
		
		// if the aspect has already been added, set the properties
		if (nodeService.hasAspect(actionedUponNodeRef,
				 QName.createQName(
						Model.NAMESPACE_CONTENT_MODEL,
						Model.ASPECT_SC_WEBABLE))) {
			...
			nodeService.addAspect(actionedUponNodeRef, QName.createQName(Model.NAMESPACE_CONTENT_MODEL, Model.ASPECT_SC_WEBABLE), properties);
		}                  
